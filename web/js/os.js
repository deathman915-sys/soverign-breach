...
function renderMemoryBanks(el, data) {
    ...
    el.innerHTML = `<div style="padding:15px; height:100%; display:flex; flex-direction:column; background:rgba(0,5,10,0.8);"><div style="color:var(--cyan); font-weight:bold; margin-bottom:15px; display:flex; justify-content:space-between; align-items:center;"><span>VFS MEMORY BANKS</span><span style="font-size:9px; color:var(--text-dim);">${files.length} FILES</span></div><div style="flex:1; overflow-y:auto;"><div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(85px, 1fr)); gap:20px; padding:10px;">${files.map(f => `<div class="vfs-icon" style="display:flex; flex-direction:column; align-items:center; gap:8px; cursor:pointer;" onclick="toggleTool('${f.id}')" oncontextmenu="event.preventDefault(); deleteVFSFile('${f.name}')"><div style="width:44px; height:44px; background:${f.loaded ? 'var(--cyan)' : '#111'}; border:1px solid ${f.loaded ? '#fff' : 'var(--p-blue)'}; display:flex; align-items:center; justify-content:center; font-size:18px; font-weight:bold; color:${f.loaded ? '#000' : '#888'}; box-shadow:${f.loaded ? '0 0 15px var(--cyan)' : 'none'};">${f.name.endsWith('.exe') ? 'EXE' : 'DAT'}</div><div style="font-size:9px; font-weight:500; color:${f.loaded ? '#fff' : '#bbb'}; text-align:center; word-break:break-all; max-width:85px; line-height:1.2;">${f.name}<br><span style="font-size:7px; opacity:0.7;">VERSION ${f.version}</span></div></div>`).join('')}</div></div><div style="border-top:1px solid #111; padding-top:10px; margin-top:10px; font-size:8px; color:var(--text-dim);">LEFT CLICK: LOAD/UNLOAD | RIGHT CLICK: DELETE</div></div>`;
}
