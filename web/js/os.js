...
async function initLogin() {
    ...
    const refreshUserList = async () => {
        if (!userList) return [];
        userList.innerHTML = '<div style="color:#666; padding:8px; font-style:italic;">Querying Uplink database...</div>';
        try {
            ...
            if (!Array.isArray(profiles) || profiles.length === 0) {
                userList.innerHTML = '<div style="color:#444; padding:8px; font-style:italic;">No profiles found</div>';
                return [];
            }
...
function renderPasswordScreen(el, s) {
    el.innerHTML = `<div style="padding:40px; text-align:center;"><div style="color:var(--red); font-weight:bold;">PASSWORD REQUIRED</div><div style="color:#444; font-size:9px; margin-top:10px;">${s.hint || ''}</div></div>`;
}
...
async function initMapApp(data) {
    ...
    try {
        ...
        mapInstance = L.map('map-app-container', { center:[30,0], zoom:2, attributionControl:false, maxBounds: [[-85, -200], [85, 200]], maxBoundsViscosity: 1.0 });
        setTimeout(() => mapInstance.invalidateSize(), 100);
        L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', { noWrap:true }).addTo(mapInstance);
        lastMapNodes.forEach(n => {
            if (!n.x || !n.y) return;
...
function renderMenuScreen(el, s) { el.innerHTML = `<div style="padding:20px;"><div style="color:var(--cyan); font-weight:bold; margin-bottom:15px; font-size:14px;">${s.title}</div><div style="display:flex; flex-direction:column; gap:5px;">${s.options.map(o => `<button class="menu-btn" style="text-align:left;" onclick="remoteNavigate(${o.screen_type})">${o.name}</button>`).join('')}</div></div>`; }
async function remoteNavigate(type) { await eel.navigate_screen(currentRemoteIp, type)(); refreshRemote(); }
function renderLogScreen(el, s) { el.innerHTML = `<div style="padding:15px;"><div style="color:var(--yellow); font-weight:bold; margin-bottom:10px;">LOGS</div>${s.logs.map(log_entry => `<div style="display:flex; justify-content:space-between; font-family:monospace; font-size:9px; border-bottom:1px solid #111; padding:4px 0;"><span style="color:var(--red);">${log_entry.subject}</span><span style="color:var(--cyan); cursor:pointer;" onclick="interactWithIP('${log_entry.from}')">${log_entry.from}</span><span>${log_entry.time}</span></div>`).join('') || 'Empty'}</div>`; }
function renderRecordScreen(el, s, title, theme) { const records = s.recordbank || []; el.innerHTML = `<div style="padding:15px; display:grid; grid-template-columns: 200px 1fr; gap:15px; height:100%;"><div style="border-right:1px solid #111; overflow-y:auto; background:rgba(0,0,0,0.2);"><div style="color:var(--${theme}); font-weight:bold; margin-bottom:10px;">${title}</div>${records.map((r, i) => `<div class="user-name" style="padding:4px 8px; border-bottom:1px solid #111; cursor:pointer;" onclick="viewRecord(${i}, '${theme}')">${r.name}</div>`).join('')}</div><div id="record-detail" style="padding:10px; color:#444;">SELECT RECORD</div></div>`; }
...
function renderFinance(el, data) { const accs = data.accounts || []; el.innerHTML = `<div style="padding:15px; height:100%; overflow-y:auto;"><div style="color:var(--cyan); font-weight:bold; margin-bottom:15px;">FINANCES</div>${accs.map(a => `<div style="border:1px solid var(--p-blue); padding:12px; margin-bottom:8px; display:flex; justify-content:space-between;"><div><div style="color:#fff;">#${a.account_number}</div><div style="color:#444; font-size:8px;">${a.bank_ip}</div></div><div style="color:var(--green); font-size:14px;">${a.balance.toLocaleString()}c</div></div>`).join('') || '<div style="color:#444; text-align:center;">No accounts linked</div>'}</div>`; }
function renderHardware(el, data) {
    ...
    el.innerHTML = `<div style="padding:15px; display:grid; grid-template-columns: 200px 1fr; gap:15px; height:100%;"><div style="border-right:1px solid #111;"><div style="color:var(--cyan); font-weight:bold; margin-bottom:10px;">${data.name || 'GATEWAY'}</div>${data.is_melted ? '<div style="color:var(--red); font-weight:bold;">MELTDOWN</div>' : `
        <div style="font-size:8px; color:#666; margin-bottom:2px;">THERMALS: ${data.heat.toFixed(1)}°C</div><div style="background:#111; height:4px; margin-bottom:6px;"><div style="width:${heatPct}%; height:100%; background:var(--orange);"></div></div>
        <div style="font-size:8px; color:#666; margin-bottom:2px;">POWER: ${data.power_draw.toFixed(1)}W</div><div style="background:#111; height:4px; margin-bottom:6px;"><div style="width:${powerPct}%; height:100%; background:var(--green);"></div></div>
        <div style="font-size:8px; color:#666; margin-bottom:2px;">ACTIVE RAM: ${ramUsed}/${ramTotal}GQ</div><div style="background:#111; height:4px; margin-bottom:6px;"><div style="width:${ramPct}%; height:100%; background:var(--cyan);"></div></div>
        <div style="font-size:8px; color:#666; margin-bottom:2px;">VFS STORAGE: ${storageUsed}/${storageTotal}GQ</div><div style="background:#111; height:4px; margin-bottom:6px;"><div style="width:${storagePct}%; height:100%; background:var(--p-blue);"></div></div>
...
function renderMessages(el, data) { const m = data.messages || []; el.innerHTML = `<div style="padding:15px; height:100%; overflow-y:auto;">${m.map(msg => `<div style="border:1px solid #222; padding:10px; margin-bottom:5px; cursor:pointer;"><div style="display:flex; justify-content:space-between; font-weight:bold;"><span>${msg.from}</span><span style="color:#444;">Tick ${msg.created_at_tick}</span></div><div style="color:var(--yellow);">${msg.subject}</div></div>`).join('') || '<div style="color:#444; text-align:center;">Inbox empty</div>'}</div>`; }
function renderLogistics(el, data) { const m = data.manifests || []; el.innerHTML = `<div style="padding:15px; height:100%; overflow-y:auto;"><div style="color:var(--yellow); font-weight:bold; margin-bottom:10px;">GLOBAL SHIPMENTS</div>${m.map(x => `<div style="border:1px solid #222; padding:10px; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center; background:rgba(0,10,20,0.3);"><div><div style="color:#fff; font-weight:bold;">${x.cargo}</div><div style="color:#666; font-size:9px;">${x.origin} -> ${x.destination}</div><div style="color:var(--cyan); font-size:8px; margin-top:4px;">Carrier: ${x.carrier} | Sec: ${x.security.toFixed(1)}</div></div><div style="text-align:right;"><div style="color:var(--orange); font-weight:bold;">${(x.progress*100).toFixed(1)}%</div><div style="color:#444; font-size:8px;">${x.vehicle_type}</div>${x.vehicle_ip ? `<button class="menu-btn" style="font-size:7px; padding:1px 4px; margin-top:4px;" onclick="interactWithIP('${x.vehicle_ip}')">HACK VEHICLE</button>` : ''}</div></div>`).join('') || '<div style="color:#444; text-align:center; padding-top:20px;">No shipments currently tracked.</div>'}</div>`; }
...
function renderCompany(el, data) {
    if (!data.owned) { el.innerHTML = `<div style="padding:40px; text-align:center;"><div style="color:var(--yellow); font-weight:bold; font-size:14px; margin-bottom:15px;">CORPORATE REGISTRATION</div><input id="new-comp-name" placeholder="COMPANY NAME" style="background:#000; border:1px solid #0af; color:#fff; padding:8px; margin-bottom:15px; outline:none; text-align:center;" /><br><div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; max-width:300px; margin:0 auto;"><button class="menu-btn" onclick="foundCompany(3)">PMC</button><button class="menu-btn" onclick="foundCompany(2)">LOGISTICS</button></div><div style="color:#444; font-size:9px; margin-top:15px;">Cost: 10,000c</div></div>`; return; }
    const squads = data.squads || [], vehicles = data.vehicles || [], manifests = data.manifests || [];
    el.innerHTML = `<div style="padding:15px; height:100%; display:flex; flex-direction:column; overflow-y:auto;"><div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:15px;"><div><div style="color:var(--cyan); font-weight:bold; font-size:16px;">${data.name}</div><div style="font-size:9px; color:#444;">TYPE: ${data.type} | STOCK: ${data.stock_price.toFixed(2)}c</div></div><div style="text-align:right;"><div style="color:var(--green); font-weight:bold; font-size:14px;">${data.balance.toLocaleString()}c</div><div style="font-size:8px; color:#444;">TREASURY</div></div></div>${data.type === 'PMC' ? `<div style="margin-bottom:20px;"><div style="color:var(--yellow); font-size:10px; font-weight:bold; margin-bottom:8px; border-bottom:1px solid #222; padding-bottom:4px;">TACTICAL SQUADS</div>${squads.map(s => `<div style="border:1px solid #222; padding:8px; margin-bottom:5px; background:rgba(255,0,0,0.05); display:flex; justify-content:space-between; align-items:center;"><div><div style="color:#fff; font-weight:bold;">${s.name}</div><div style="color:#666; font-size:8px;">Combat: ${s.combat.toFixed(1)} | Loc: ${s.location}</div></div><div style="text-align:right;"><div style="color:var(--cyan); font-size:9px;">${s.status}</div></div></div>`).join('') || '<div style="color:#444; font-size:9px; text-align:center;">No squads enlisted.</div>'}</div>` : ''}<div style="margin-bottom:20px;"><div style="color:var(--yellow); font-size:10px; font-weight:bold; margin-bottom:8px; border-bottom:1px solid #222; padding-bottom:4px;">ACTIVE FLEET</div><div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap:5px;">${vehicles.map(v => `<div style="border:1px solid #222; padding:5px; text-align:center; background:rgba(0,170,255,0.05);"><div style="color:#fff; font-size:9px; font-weight:bold;">${v.name}</div><div style="color:#444; font-size:8px;">${v.type} | ${v.status}</div></div>`).join('') || '<div style="color:#444; font-size:9px; grid-column: 1 / -1; text-align:center;">No active assets.</div>'}</div></div><div><div style="color:var(--yellow); font-size:10px; font-weight:bold; margin-bottom:8px; border-bottom:1px solid #222; padding-bottom:4px;">ACTIVE CONTRACTS</div>${manifests.map(m => `<div style="border:1px solid #222; padding:8px; margin-bottom:5px; display:flex; justify-content:space-between; align-items:center;"><div><div style="color:#fff; font-size:10px;">${m.cargo}</div><div style="color:#444; font-size:8px;">${m.origin} -> ${m.destination}</div></div><div style="text-align:right;"><div style="color:var(--orange); font-size:10px; font-weight:bold;">${(m.progress*100).toFixed(1)}%</div></div></div>`).join('') || '<div style="color:#444; font-size:9px; text-align:center;">No active contracts.</div>'}</div></div></div>`;
}

function renderBBSScreen(el, s) { const missions = s.missions || []; el.innerHTML = `<div style="padding:15px; overflow-y:auto; height:100%; background:rgba(0,10,20,0.4);"><div style="color:var(--cyan); font-weight:bold; margin-bottom:15px; border-bottom:1px solid #222; padding-bottom:5px;">PUBLIC MISSION BOARD</div>${missions.map(m => `<div style="border:1px solid #222; padding:12px; margin-bottom:8px; background:rgba(0,0,0,0.3); display:flex; justify-content:space-between; align-items:center;"><div><div style="color:#fff; font-size:11px; font-weight:bold; margin-bottom:3px;">${m.employer || m.type}</div><div style="color:#666; font-size:9px; line-height:1.2;">${m.description || ''}</div><div style="color:var(--green); font-size:10px; margin-top:5px; font-family:monospace;">${m.payment.toLocaleString()} Credits</div></div><div style="display:flex; flex-direction:column; gap:4px;"><button class="menu-btn" style="font-size:9px;" onclick="serverAcceptMission(${m.id})">ACCEPT</button><button class="menu-btn" style="font-size:9px; background:#222;" onclick="serverNegotiateMission(${m.id})">NEGOTIATE</button></div></div>`).join('') || '<div style="color:#333; text-align:center; padding-top:20px;">No public contracts listed.</div>'}</div>`; }
function renderStore(el, data) {
    ...
    el.innerHTML = `<div style="padding:15px; height:100%; overflow-y:auto; background:rgba(0,10,20,0.5);"><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; border-bottom:1px solid #222; padding-bottom:8px;"><div style="color:var(--cyan); font-weight:bold; font-size:14px;">HARDWARE & SOFTWARE STORE</div><div style="color:var(--green); font-size:12px; font-weight:bold;">${balance.toLocaleString()}c</div></div><div class="store-grid" style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
        <div>
            ...
            ${software.map(s => `<div style="border:1px solid #222; padding:8px; margin-bottom:5px; background:rgba(0,0,0,0.3); display:flex; justify-content:space-between; align-items:center;"><div><div style="color:#fff; font-size:10px;">${s.name} v${s.version}</div><div style="color:#444; font-size:8px;">${s.size}GQ</div></div><button class="menu-btn" style="font-size:9px;" onclick="serverBuySoftware('${s.name}', ${s.version})">${s.price}c</button></div>`).join('')}
        </div>
        <div>
...
async function showHardwareCategory(cat) {
    ...
    list.innerHTML = '<div style="color:#444; font-size:9px;">Loading catalog...</div>';
